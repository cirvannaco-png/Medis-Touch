...
from app.models import ExperimentObservation, PromotionDecisionAudit, ResearchExperiment
...
class ReserveIn(BaseModel):
    ...
    max_heat_r: float = Field(gt=0, default=3.0)
...
async def create_experiment(
    ...
    if await session.scalar(
        select(ResearchExperiment).where(
            ResearchExperiment.experiment_id == body.experiment_id
        )
    ):
        raise HTTPException(409, "experiment already exists")
    if (
        body.train_start >= body.train_end
        or body.oos_start >= body.oos_end
        or body.train_end > body.oos_start
    ):
        raise HTTPException(422, "invalid train/OOS chronology")
    row = ResearchExperiment(
        experiment_id=body.experiment_id,
        name=body.name,
        code_sha=body.code_sha,
        config_hash=canonical_hash(body.config),
        data_snapshot=body.data_snapshot,
        created_at=datetime.now(timezone.utc),
        train_start=body.train_start,
        train_end=body.train_end,
        oos_start=body.oos_start,
        oos_end=body.oos_end,
        walk_forward_id=body.walk_forward_id,
        holdout_locked=body.holdout_locked,
    )
    session.add(row)
    ...
    return {
        ...
        "holdout_locked": row.holdout_locked,
    }
...
async def ingest_observation(
    ...
    if (
        await session.scalar(
            select(ResearchExperiment).where(
                ResearchExperiment.experiment_id == experiment_id
            )
        )
        is None
    ):
...
async def promotion_gate(
    ...
    policy = GatePolicy(
        ...
        min_expectancy_r=body.min_expectancy_r,
        max_drawdown_r=body.max_drawdown_r,
        min_win_rate=body.min_win_rate,
        min_effect_size_r=body.min_effect_size_r,
    )
    decision = gate(
        body.champion_r,
        body.challenger_r,
    ...
    if body.holdout_passed and not exp.holdout_locked:
        raise HTTPException(
            409,
            "holdout must be locked before accepting a passing result",
        )
    audit = PromotionDecisionAudit(
        ...
        metrics_json=decision.__dict__,
...
async def execution_reserve(
    ...
    if not ok:
        ...
        raise HTTPException(
            409,
            "request duplicate or portfolio heat limit exceeded",
        )
    await session.commit()
    return {"reserved": True, "request_id": body.request_id}
...
async def execution_transition(
    ...
    if not ok:
        await session.rollback()
        raise HTTPException(409, "unknown or terminal execution request")
    await session.commit()
    return {
        "transitioned": True,
...
async def execution_recover(
    session: AsyncSession = Depends(get_session),
    _auth: bool = Depends(verify_api_key),
):
    count = await recover_expired_leases(session)
    await session.commit()
